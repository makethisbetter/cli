...
func TestPendingLoginRoundTripUsesOwnerOnlyFile(t *testing.T) {
	configPath := filepath.Join(t.TempDir(), ".makethisbetter", "config.json")
	t.Setenv("MAKETHISBETTER_CONFIG", configPath)
	...
	if err := SavePendingLogin(pending); err != nil {
		t.Fatalf("SavePendingLogin failed: %v", err)
	}
	loaded, err := LoadPendingLogin()
	if err != nil {
		t.Fatalf("LoadPendingLogin failed: %v", err)
	}
	if *loaded != *pending {
		t.Fatalf("loaded pending login = %#v, want %#v", loaded, pending)
	}
	...
	if perm := info.Mode().Perm(); perm != 0o600 {
		t.Fatalf("pending login permissions = %o, want 600", perm)
	}

	if err := RemovePendingLogin(); err != nil {
		t.Fatalf("RemovePendingLogin failed: %v", err)
	}
	if _, err := os.Stat(path); !os.IsNotExist(err) {
		t.Fatalf("pending login file still exists: %v", err)
	}
}
...
// A hand-created config (the README shows its contents) is 0644 under a default
// umask, and os.WriteFile ignores perm for a file that already exists — so
// saving a token over it must tighten the mode rather than inherit it.
func TestSaveTo_TightensExistingLoosePermissions(t *testing.T) {
	dir := filepath.Join(t.TempDir(), ".makethisbetter")
	if err := os.MkdirAll(dir, 0o755); err != nil {
		t.Fatal(err)
	}
	path := filepath.Join(dir, "config.json")
	if err := os.WriteFile(path, []byte(`{"api_url":"https://makethisbetter.dev/api/v1"}`), 0o644); err != nil {
		t.Fatal(err)
	}

	if err := SaveTo(&Config{Token: "secret", APIURL: DefaultAPIURL}, path); err != nil {
		t.Fatalf("SaveTo failed: %v", err)
	}
