...
func FeedbackTable(w io.Writer, feedbacks []api.Feedback) {
	feedbackTable(w, feedbacks, "Updated", func(feedback api.Feedback) string { return feedback.UpdatedAt })
}

// ArchivedFeedbackTable writes archived feedbacks with their archive time while
// preserving the underlying workflow status.
func ArchivedFeedbackTable(w io.Writer, feedbacks []api.Feedback) {
	feedbackTable(w, feedbacks, "Archived", func(feedback api.Feedback) string {
		return ptrOr(feedback.ArchivedAt, "-")
	})
}

func feedbackTable(w io.Writer, feedbacks []api.Feedback, timestampHeader string, timestamp func(api.Feedback) string) {
	if len(feedbacks) == 0 {
	...
	rows = append(rows, []string{"ID", "Status", "Priority", "Labels", "Description", timestampHeader})
	for _, fb := range feedbacks {
		rows = append(rows, []string{
			fb.Reference,
			fb.Status,
			fb.Priority,
			strings.Join(fb.Labels, ", "),
			truncate(fb.Description, 48),
			formatDate(timestamp(fb)),
		})
	}
...
func FeedbackDetail(w io.Writer, fb *api.Feedback) {
	...
	labels := "-"
	if len(fb.Labels) > 0 {
		labels = strings.Join(fb.Labels, ", ")
	}
	fmt.Fprintf(w, "%s %s\n", Dim("Labels:"), labels)
	fmt.Fprintf(w, "%s %s\n", Dim("Recommendation:"), ptrOr(fb.Recommendation, "-"))
	fmt.Fprintf(w, "%s %s\n", Dim("Close reason:"), ptrOr(fb.CloseReason, "-"))
	fmt.Fprintf(w, "%s %s\n", Dim("Pull request:"), ptrOr(fb.PRURL, "-"))
	if fb.Assignee != nil {
		fmt.Fprintf(w, "%s %s\n", Dim("Assignee:"), fb.Assignee.Name)
	}
	fmt.Fprintf(w, "%s %s\n", Dim("Page:"), ptrOr(fb.PageURL, "-"))
	fmt.Fprintf(w, "%s %s\n", Dim("Reporter:"), formatReporter(fb))
	if viewport := formatViewport(fb); viewport != "" {
		fmt.Fprintf(w, "%s %s\n", Dim("Viewport:"), viewport)
	}
	if fb.ReporterLanguage != nil && *fb.ReporterLanguage != "" {
		fmt.Fprintf(w, "%s %s\n", Dim("Language:"), *fb.ReporterLanguage)
	}
	if triage := formatTriageStatus(fb); triage != "" {
		fmt.Fprintf(w, "%s %s\n", Dim("AI triage:"), triage)
	}
	if clarification := formatClarificationStatus(fb); clarification != "" {
		fmt.Fprintf(w, "%s %s\n", Dim("AI clarification:"), clarification)
	}
	fmt.Fprintf(w, "%s %s\n", Dim("Created:"), formatDate(fb.CreatedAt))
	fmt.Fprintf(w, "%s %s\n", Dim("Updated:"), formatDate(fb.UpdatedAt))
	if fb.ArchivedAt != nil {
		fmt.Fprintf(w, "%s %s\n", Dim("Archived:"), formatDate(*fb.ArchivedAt))
	}

	if fb.ScreenshotURL != nil && *fb.ScreenshotURL != "" {
		fmt.Fprintf(w, "%s %s\n", Dim("Screenshot:"), *fb.ScreenshotURL)
	} else if fb.ScreenshotAttached {
		// Older self-hosted servers only expose whether an attachment exists.
	...
	if len(fb.Annotations) > 0 {
		fmt.Fprintln(w)
		fmt.Fprintln(w, Bold("Annotations"))
		for _, a := range fb.Annotations {
			fmt.Fprintf(w, "- %s\n", formatAnnotation(a))
		}
	}

	if len(fb.Breadcrumbs) > 0 {
		fmt.Fprintln(w)
		fmt.Fprintln(w, Bold("Breadcrumbs"))
		for _, b := range fb.Breadcrumbs {
			fmt.Fprintf(w, "- %s\n", string(b))
		}
	}

	if len(fb.AIClarificationMessages) > 0 {
		fmt.Fprintln(w)
		fmt.Fprintln(w, Bold("AI clarification"))
		for _, m := range fb.AIClarificationMessages {
			fmt.Fprintf(w, "%s %s\n", Dim(m.Role+":"), m.Content)
		}
	}
...
func ProjectDetail(w io.Writer, p *api.Project) {
	...
	if p.SigningSecret != nil {
		...
		fmt.Fprintf(w, "%s %s\n", Dim("Signing secret:"), Dim("(not available for this role)"))
	}
...
func formatViewport(fb *api.Feedback) string {
	if fb.ScreenWidth == nil || fb.ScreenHeight == nil {
		return ""
	}
	return fmt.Sprintf("%dx%d", *fb.ScreenWidth, *fb.ScreenHeight)
}

// formatTriageStatus keeps a failed triage from reading like an untriaged one.
// Both lack an AI summary; only the status says which happened.
func formatTriageStatus(fb *api.Feedback) string {
	if fb.AITriageStatus == nil || *fb.AITriageStatus == "" {
		return ""
	}
	status := *fb.AITriageStatus
	switch status {
	case "failed", "credits_exhausted":
		line := Red(status)
		if fb.AITriageError != nil && *fb.AITriageError != "" {
			line += " " + Dim(truncate(*fb.AITriageError, 120))
		}
		return line
	case "completed":
		return Green(status)
...
// formatClarificationStatus flags the case a triager needs to notice: the AI
// asked the reporter a question and the answer never came back, so the
// description on screen is not the whole story yet.
func formatClarificationStatus(fb *api.Feedback) string {
	if fb.AIClarificationStatus == nil || *fb.AIClarificationStatus == "" {
		return ""
	}
	status := *fb.AIClarificationStatus
	if status == "active" {
		return Yellow(status + " (question sent, reporter has not replied)")
	}
	return status
}

// formatAnnotation names what the reporter pointed at, falling back to the
// selector and then to raw coordinates when the widget captured no element.
func formatAnnotation(a api.Annotation) string {
	target := a.TargetName
	if target == "" {
		target = a.TargetSelector
	}
	if target == "" {
		target = fmt.Sprintf("(%.0f, %.0f)", a.X, a.Y)
	}
	if a.Type != "" {
		target = a.Type + ": " + target
	}
	if a.TargetText != "" {
		target += " " + Dim(fmt.Sprintf("%q", truncate(a.TargetText, 60)))
	}
	return target
}
