...
func TestProjectTableRows(t *testing.T) {
	...
	got := buf.String()
	if !strings.Contains(got, "project_1") || !strings.Contains(got, "Acme") || !strings.Contains(got, "3") {
...
func TestPrintProjectResultJSON(t *testing.T) {
	var buf bytes.Buffer
	p := &api.Project{ID: "project_1", Name: "Acme"}
	PrintProjectResult(&buf, p, true, "ignored message")
	got := buf.String()
	if !strings.Contains(got, `"project_1"`) {
		t.Errorf("expected JSON with project_1, got %q", got)
	}
	if strings.Contains(got, "ignored message") {
		t.Error("message should not appear in JSON mode")
	}
...
func TestPrintProjectResultFormatted(t *testing.T) {
	var buf bytes.Buffer
	p := &api.Project{ID: "project_1", Name: "Acme", APIKey: "mtb_proj_abc"}
...
func TestProjectDetailSigningSecretAbsent(t *testing.T) {
	var buf bytes.Buffer
	ProjectDetail(&buf, &api.Project{ID: "project_1", Name: "Acme"})
	got := buf.String()
...
func TestProjectDetailSigningSecretPresent(t *testing.T) {
	secret := "whsec_abc"
	var buf bytes.Buffer
	ProjectDetail(&buf, &api.Project{ID: "project_1", Name: "Acme", SigningSecret: &secret})
	got := buf.String()
