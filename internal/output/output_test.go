...
// A Chinese description must not be cut mid-rune, and its column has to reach
// the same visible width as an ASCII one or every following column shifts.
func TestTableHandlesCJK(t *testing.T) {
	const chinese = "结账按钮在手机上点了没有任何反应，换成桌面浏览器就正常。"

	cut := truncate(chinese, 12)
	if !utf8.ValidString(cut) {
		t.Errorf("truncate produced invalid UTF-8: %q", cut)
	}
	if strings.ContainsRune(cut, utf8.RuneError) {
		t.Errorf("truncate produced a replacement character: %q", cut)
	}
	if !strings.HasSuffix(cut, "...") {
		t.Errorf("expected an ellipsis on a truncated cell, got %q", cut)
	}
	if got := displayWidth(cut); got > 12 {
		t.Errorf("truncated cell is %d columns wide, want at most 12", got)
	}
	...
	widths := columnWidths(rows, []int{0})
	if widths[0] != 11 {
		t.Errorf("column width: got %d, want 11 (the ASCII header is widest)", widths[0])
	}
	if got := displayWidth(padRight("结账按钮", widths[0])); got != 11 {
		t.Errorf("padded CJK cell is %d columns wide, want 11", got)
	}
}
...
func TestArchivedFeedbackOutputKeepsStatusAndShowsArchiveTime(t *testing.T) {
	archivedAt := "2026-08-03T12:00:00Z"
	feedback := api.Feedback{Reference: "acme/FB-1", Status: "received", ArchivedAt: &archivedAt}

	var table bytes.Buffer
	ArchivedFeedbackTable(&table, []api.Feedback{feedback})
	if !strings.Contains(table.String(), "Archived") || !strings.Contains(table.String(), "received") {
		t.Fatalf("archived table = %q", table.String())
	}
	if strings.Contains(table.String(), "Updated") {
		t.Fatalf("archived table should not use Updated column: %q", table.String())
	}

	var detail bytes.Buffer
	FeedbackDetail(&detail, &feedback)
	if !strings.Contains(detail.String(), "Archived:") || !strings.Contains(detail.String(), "received") {
		t.Fatalf("archived detail = %q", detail.String())
	}
}
...
func TestProjectDetailSigningSecretAbsent(t *testing.T) {
	...
	got := buf.String()
	if !strings.Contains(got, "not available for this role") {
		t.Errorf("expected role-aware note when signing secret absent, got %q", got)
	}
}
...
func TestFeedbackDetailShowsAnnotationTarget(t *testing.T) {
	var buf bytes.Buffer
	fb := &api.Feedback{
		Reference: "acme/FB-9",
		Annotations: []api.Annotation{
			{X: 10, Y: 20, Type: "point", TargetName: "Save button", TargetText: "Save changes", TargetSelector: "#save"},
		},
	}
	FeedbackDetail(&buf, fb)

	out := buf.String()
	if !strings.Contains(out, "Annotations") {
		t.Error("detail should have an Annotations section")
	}
	if !strings.Contains(out, "Save button") {
		t.Errorf("detail should name what the reporter pointed at, got:\n%s", out)
	}
	if !strings.Contains(out, "Save changes") {
		t.Errorf("detail should include the annotated element's text, got:\n%s", out)
	}
}

func TestFeedbackDetailShowsScreenshotURL(t *testing.T) {
	var buf bytes.Buffer
	screenshotURL := "https://cdn.example.com/screenshots/FB-9.jpg"
	fb := &api.Feedback{Reference: "acme/FB-9", ScreenshotAttached: true, ScreenshotURL: &screenshotURL}

	FeedbackDetail(&buf, fb)

	if !strings.Contains(buf.String(), screenshotURL) {
		t.Errorf("detail should show the screenshot URL, got:\n%s", buf.String())
	}
}

func TestFeedbackDetailShowsAssignee(t *testing.T) {
	var buf bytes.Buffer
	fb := &api.Feedback{Reference: "acme/FB-9", Assignee: &api.FeedbackAssignee{ID: "usr_1", Name: "Alex"}}

	FeedbackDetail(&buf, fb)

	if !strings.Contains(buf.String(), "Assignee: Alex") {
		t.Errorf("detail should show assignee, got:\n%s", buf.String())
	}
}

func TestFeedbackDetailFallsBackToSelectorThenCoordinates(t *testing.T) {
	var buf bytes.Buffer
	fb := &api.Feedback{
		Reference: "acme/FB-9",
		Annotations: []api.Annotation{
			{X: 10, Y: 20, TargetSelector: "#only-selector"},
			{X: 33, Y: 44},
		},
	}
	FeedbackDetail(&buf, fb)
	...
	out := buf.String()
	if !strings.Contains(out, "#only-selector") {
		t.Errorf("annotation without a name should fall back to its selector, got:\n%s", out)
	}
	if !strings.Contains(out, "(33, 44)") {
		t.Errorf("annotation without a target should fall back to coordinates, got:\n%s", out)
	}
}

func TestFeedbackDetailFlagsUnansweredClarification(t *testing.T) {
	var buf bytes.Buffer
	active := "active"
	fb := &api.Feedback{
		Reference:             "acme/FB-9",
		AIClarificationStatus: &active,
		AIClarificationMessages: []api.ClarificationMessage{
	...
	out := buf.String()
	if !strings.Contains(out, "AI clarification:") {
		t.Errorf("detail should report the clarification status, got:\n%s", out)
	}
	if !strings.Contains(out, "has not replied") {
		t.Errorf("an active clarification should be spelled out as unanswered, got:\n%s", out)
	}
	if !strings.Contains(out, "Which button did you press?") {
		t.Errorf("detail should include the clarification transcript, got:\n%s", out)
	}
}

func TestFeedbackDetailShowsFailedTriageWithError(t *testing.T) {
	var buf bytes.Buffer
	status := "failed"
	msg := "Anthropic API timeout"
	fb := &api.Feedback{Reference: "acme/FB-9", AITriageStatus: &status, AITriageError: &msg}
	FeedbackDetail(&buf, fb)

	out := buf.String()
	if !strings.Contains(out, "failed") || !strings.Contains(out, msg) {
		t.Errorf("a failed triage should be distinguishable from an untriaged one, got:\n%s", out)
	}
}

func TestFeedbackDetailOmitsAbsentEvidence(t *testing.T) {
	var buf bytes.Buffer
	FeedbackDetail(&buf, &api.Feedback{Reference: "acme/FB-9"})

	out := buf.String()
	for _, section := range []string{"Annotations", "Breadcrumbs", "AI clarification", "Viewport:", "Language:", "AI triage:"} {
		if strings.Contains(out, section) {
			t.Errorf("%q should not appear when the API sent nothing for it, got:\n%s", section, out)
		}
	}
}

func TestFeedbackDetailShowsViewportAndLanguage(t *testing.T) {
	var buf bytes.Buffer
	w, h := 1440, 900
	lang := "zh-CN"
	fb := &api.Feedback{Reference: "acme/FB-9", ScreenWidth: &w, ScreenHeight: &h, ReporterLanguage: &lang}
	FeedbackDetail(&buf, fb)

	out := buf.String()
	if !strings.Contains(out, "1440x900") {
		t.Errorf("detail should show the viewport, got:\n%s", out)
	}
	if !strings.Contains(out, "zh-CN") {
		t.Errorf("detail should show the reporter language, got:\n%s", out)
	}
}
