...
func TestResolveFeedbackUsesV2ResolutionResource(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != "POST" {
			t.Errorf("expected POST, got %s", r.Method)
		}
		if r.URL.Path != "/api/v2/projects/acme/feedbacks/1/resolution" {
			t.Errorf("expected v2 resolution path, got %s", r.URL.Path)
		}

		var body map[string]json.RawMessage
		json.NewDecoder(r.Body).Decode(&body)
		var resolution map[string]any
		json.Unmarshal(body["feedback_resolution"], &resolution)
		if resolution["resolution_summary"] != "Fixed Safari export." {
			t.Errorf("unexpected resolution summary: %v", resolution["resolution_summary"])
		}
		if resolution["pr_url"] != "https://github.com/acme/app/pull/1" {
			t.Errorf("unexpected PR URL: %v", resolution["pr_url"])
		}
	...
	result, err := client.ResolveFeedback(context.Background(), "acme/FB-1", ResolveFeedbackParams{
		ResolutionSummary: "Fixed Safari export.",
