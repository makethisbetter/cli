...
func TestListFeedbacks(t *testing.T) {
	...
	result, err := client.ListFeedbacks(context.Background(), ListFeedbacksParams{ProjectHandle: "acme", Limit: DefaultListLimit})
	if err != nil {
...
func TestListFeedbacksRejectsOutOfRangeLimitBeforeRequest(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Error("invalid limit should not make an HTTP request")
	}))
	defer server.Close()

	client := NewClient(&config.Config{Token: "tok", APIURL: server.URL})
	_, err := client.ListFeedbacks(context.Background(), ListFeedbacksParams{ProjectHandle: "acme", Limit: MaxListLimit + 1})
	if err == nil || !strings.Contains(err.Error(), "limit must be between 1 and 200") {
		t.Fatalf("expected bounded limit error, got %v", err)
	}
	_, err = client.ListFeedbacks(context.Background(), ListFeedbacksParams{ProjectHandle: "acme", Limit: 0})
	if err == nil || !strings.Contains(err.Error(), "limit must be between 1 and 200") {
...
func TestUpdateFeedback(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if fb["takeover"] != true {
			t.Errorf("expected takeover=true, got %v", fb["takeover"])
		}
	...
	client := NewClient(&config.Config{Token: "tok", APIURL: server.URL})
	result, err := client.UpdateFeedback(context.Background(), "acme/FB-1", UpdateFeedbackParams{Status: "in_progress", Takeover: true})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestReadyFeedbackUsesV2ReadinessResource(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != "POST" {
			t.Errorf("expected POST, got %s", r.Method)
		}
		if r.URL.Path != "/api/v2/projects/acme/feedbacks/1/readiness" {
			t.Errorf("expected v2 readiness path, got %s", r.URL.Path)
		}

		var body map[string]json.RawMessage
		json.NewDecoder(r.Body).Decode(&body)
		var readiness map[string]any
		json.Unmarshal(body["feedback_readiness"], &readiness)
		if readiness["resolution_summary"] != "Fixed Safari export." {
			t.Errorf("unexpected resolution summary: %v", readiness["resolution_summary"])
		}

		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(Feedback{ID: "FB-1", Reference: "acme/FB-1", ProjectID: "acme", Status: "pending_release"})
	}))
	defer server.Close()

	client := NewClient(&config.Config{Token: "tok", APIURL: server.URL + "/api/v1"})
	result, err := client.ReadyFeedback(context.Background(), "acme/FB-1", "Fixed Safari export.")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.Status != "pending_release" {
		t.Errorf("expected pending release feedback, got %s", result.Status)
	}
}

func TestReleaseFeedbackUsesV2ReleaseResource(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != "POST" {
			t.Errorf("expected POST, got %s", r.Method)
		}
		if r.URL.Path != "/api/v2/projects/acme/feedbacks/1/release" {
			t.Errorf("expected v2 release path, got %s", r.URL.Path)
		}
		var body map[string]json.RawMessage
		json.NewDecoder(r.Body).Decode(&body)
		var release map[string]any
		json.Unmarshal(body["feedback_release"], &release)
		if release["trailer_committed_at"] != "2026-08-01T12:00:00Z" {
			t.Errorf("unexpected trailer commit time: %v", release["trailer_committed_at"])
		}

		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(Feedback{ID: "FB-1", Reference: "acme/FB-1", ProjectID: "acme", Status: "closed"})
	}))
	defer server.Close()

	client := NewClient(&config.Config{Token: "tok", APIURL: server.URL + "/api/v1"})
	result, err := client.ReleaseFeedback(context.Background(), "acme/FB-1", "2026-08-01T12:00:00Z")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.Status != "closed" {
		t.Errorf("expected closed feedback, got %s", result.Status)
	}
}
...
func TestAPIError401(t *testing.T) {
	...
	_, err := client.ListFeedbacks(context.Background(), ListFeedbacksParams{ProjectHandle: "acme", Limit: DefaultListLimit})
	if err == nil {
...
// Rails answers authorization failures with a bare `head :forbidden` and 404s
// with no body at all, so the CLI has to supply the next action itself.
func TestAPIErrorEmptyBody(t *testing.T) {
	tests := []struct {
		status int
		want   string
	}{
		{http.StatusForbidden, "makethisbetter project list"},
		{http.StatusNotFound, "makethisbetter feedback list"},
		{http.StatusUnprocessableEntity, "check the flag values"},
	}
	...
	for _, tt := range tests {
		...
		if !strings.Contains(err.Error(), tt.want) {
			t.Errorf("status %d: got %q, want it to mention %q", tt.status, err.Error(), tt.want)
		}
		if strings.Contains(err.Error(), "failed with status") {
			t.Errorf("status %d: got the bare status fallback: %q", tt.status, err.Error())
		}
	}
}

// A real error body from the server must win over the CLI's generic fallback.
func TestAPIErrorBodyWinsOverFallback(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusForbidden)
		w.Write([]byte(`{"error":"your role cannot dismiss feedback"}`))
	}))
	defer server.Close()
	...
	if err.Error() != "your role cannot dismiss feedback" {
		t.Errorf("server message should win, got %q", err.Error())
	}
}
...
func TestUpdateProjectSendsOnlySetFields(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != "PATCH" {
			t.Errorf("expected PATCH, got %s", r.Method)
		}
		if r.URL.Path != "/projects/acme" {
			t.Errorf("expected path /projects/acme, got %s", r.URL.Path)
		}
		...
		p := body["project"]
		if p["ai_context"] != "B2B invoicing app" {
			t.Errorf("expected ai_context in body, got %v", p["ai_context"])
		}
		if _, ok := p["name"]; ok {
			t.Error("unset name should be omitted from the request body")
		}
		if _, ok := p["domain"]; ok {
			t.Error("unset domain should be omitted from the request body")
		}

		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(Project{ID: "acme", Handle: "acme", Name: "Acme"})
	}))
	defer server.Close()

	aiContext := "B2B invoicing app"
	client := NewClient(&config.Config{Token: "tok", APIURL: server.URL})
	result, err := client.UpdateProject(context.Background(), "acme", UpdateProjectParams{AIContext: &aiContext})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.Handle != "acme" {
		t.Errorf("expected handle acme, got %s", result.Handle)
	}
}

func TestUpdateProjectRejectsInvalidHandleBeforeRequest(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Error("invalid handle should not make an HTTP request")
	}))
	defer server.Close()

	name := "Acme"
	client := NewClient(&config.Config{Token: "tok", APIURL: server.URL})
	_, err := client.UpdateProject(context.Background(), "bad", UpdateProjectParams{Name: &name})
	if err == nil || !strings.Contains(err.Error(), "project handle must") {
		t.Fatalf("expected project handle validation error, got %v", err)
	}
}
...
func TestUpdateProjectError(t *testing.T) {
	...
	aiContext := "ctx"
	client := NewClient(&config.Config{Token: "tok", APIURL: server.URL})
	_, err := client.UpdateProject(context.Background(), "acme", UpdateProjectParams{AIContext: &aiContext})
	if err == nil {
		t.Fatal("expected error for 403")
	}
	var apiErr *APIError
