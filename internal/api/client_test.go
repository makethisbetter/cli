...
func TestListProjects(t *testing.T) {
	projects := []Project{
		{ID: "project_1", Name: "Acme", FeedbacksCount: 3},
	}
	...
	if result[0].ID != "project_1" {
		t.Errorf("expected ID project_1, got %s", result[0].ID)
	}
}

func TestGetProject(t *testing.T) {
	secret := "whsec_abc"
	project := Project{ID: "project_42", Name: "Acme", APIKey: "mtb_proj_abc", SigningSecret: &secret}

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/projects/project_42" {
			t.Errorf("expected /projects/project_42, got %s", r.URL.Path)
		}
		w.Header().Set("Content-Type", "application/json")
	...
	client := NewClient(&config.Config{Token: "tok", APIURL: server.URL})
	result, err := client.GetProject(context.Background(), "project_42")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.ID != "project_42" {
		t.Errorf("expected ID project_42, got %s", result.ID)
	}
...
func TestGetProjectWithoutSigningSecret(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.Write([]byte(`{"id":"project_1","name":"Acme","api_key":"mtb_proj_abc"}`))
	}))
	defer server.Close()

	client := NewClient(&config.Config{Token: "tok", APIURL: server.URL})
	result, err := client.GetProject(context.Background(), "project_1")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestCreateProject(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(201)
		json.NewEncoder(w).Encode(Project{ID: "project_1", Name: "New", APIKey: "mtb_proj_abc"})
	}))
	defer server.Close()

	client := NewClient(&config.Config{Token: "tok", APIURL: server.URL})
	result, err := client.CreateProject(context.Background(), CreateProjectParams{Name: "New", Domain: "example.com"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.Name != "New" {
...
func TestCreateProjectError(t *testing.T) {
	...
	client := NewClient(&config.Config{Token: "tok", APIURL: server.URL})
	_, err := client.CreateProject(context.Background(), CreateProjectParams{Name: "New"})
	if err == nil {
