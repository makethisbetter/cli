...
func init() {
	...
	listCmd.Flags().StringVar(&listType, "type", "", "deprecated compatibility alias for --label")
	...
	listCmd.Flags().IntVar(&listLimit, "limit", api.DefaultListLimit, "maximum number of feedback items to return (1-200)")
	listCmd.Flags().BoolVar(&listArchived, "archived", false, "list archived feedback")
	listCmd.Flags().BoolVar(&listJSON, "json", false, "print JSON output")
	listCmd.MarkFlagsMutuallyExclusive("archived", "status")
}
...
func runList(cmd *cobra.Command, args []string) error {
	...
	label, err := selectedListLabel(listLabel, listType)
	if err != nil {
		return err
	}
	...
	params := api.ListFeedbacksParams{
		Status:        listStatus,
		Label:         label,
	...
	var feedbacks []api.Feedback
	if listArchived {
		feedbacks, err = client.ListArchivedFeedbacks(cmd.Context(), params)
	} else {
		feedbacks, err = client.ListFeedbacks(cmd.Context(), params)
	}
	if err != nil {
	...
	if listJSON {
		...
		output.ArchivedFeedbackTable(os.Stdout, feedbacks)
	} else {
...
func selectedListLabel(label, legacyType string) (string, error) {
	if label != "" && legacyType != "" {
		return "", fmt.Errorf("--label and --type cannot be used together")
	}
	if label != "" {
		return label, nil
	}
