...
func init() {
	...
	releaseCmd.Flags().StringVar(&releaseProject, "project", "", "project handle (required)")
...
func runRelease(cmd *cobra.Command, _ []string) error {
	...
	result := releaseFeedbackReferences(cmd.Context(), client, trailers, releaseProject)
...
func releaseFeedbackReferences(ctx context.Context, client releaseClient, trailers []feedbackTrailer, projectHandle string) releaseResult {
	...
	for _, trailer := range trailers {
		...
		handle, _, err := api.ParseFeedbackReference(reference)
		if err != nil || handle != projectHandle {
			continue
		}
