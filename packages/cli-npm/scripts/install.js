const fs = require("fs");
const path = require("path");
const {
  binaryExtension,
  packageBinarySpecifier,
  releaseBinaryName,
} = require("./platform");

const VERSION = require("../package.json").version;

// Check if the binary is already available via optionalDependencies
try {
  var packageBinary = packageBinarySpecifier(process.platform, process.arch);
  if (packageBinary) {
    require.resolve(packageBinary);
    process.exit(0);
  }
} catch {}

var filename = releaseBinaryName(process.platform, process.arch);

if (!filename) {
  console.error(
    "@makethisbetter/cli postinstall: unsupported platform " +
      process.platform +
      "/" +
      process.arch +
      ", skipping binary download"
  );
  process.exit(0);
}

var url =
  "https://github.com/makethisbetter/cli/releases/download/v" +
  VERSION +
  "/" +
  filename;

var destDir = path.join(__dirname, "..", "downloaded");
fs.mkdirSync(destDir, { recursive: true });
var dest = path.join(
  destDir,
  "makethisbetter" + binaryExtension(process.platform)
);

function download(url, dest, redirects) {
  if (redirects > 5) {
    console.error("@makethisbetter/cli postinstall: too many redirects");
    process.exit(0);
  }

  var mod = url.startsWith("https") ? require("https") : require("http");

  mod
    .get(url, { headers: { "User-Agent": "makethisbetter-cli" } }, function (res) {
      if (
        res.statusCode >= 300 &&
        res.statusCode < 400 &&
        res.headers.location
      ) {
        return download(res.headers.location, dest, redirects + 1);
      }

      if (res.statusCode !== 200) {
        console.error(
          "@makethisbetter/cli postinstall: download failed with status " +
            res.statusCode
        );
        process.exit(0);
      }

      var file = fs.createWriteStream(dest);
      res.pipe(file);
      file.on("finish", function () {
        file.close();
        fs.chmodSync(dest, 0o755);
      });
    })
    .on("error", function () {
      // Silent fail — user can install via go install
      process.exit(0);
    });
}

download(url, dest, 0);
