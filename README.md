...
makethisbetter feedback release --through <deployed-sha>
```
...
releases only `pending_release` Feedback whose latest trailer is reachable from
the deployed commit. Released Feedback becomes `closed(shipped)`; the platform
then generates and sends each Reporter Update.
