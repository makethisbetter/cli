...
makethisbetter feedback resolve acme/FB-1  # mark shipped, user gets notified
```
...
makethisbetter feedback list --project acme --status received --type bug --priority high
...
```bash
makethisbetter feedback dismiss acme/FB-1 --reason duplicate
```
